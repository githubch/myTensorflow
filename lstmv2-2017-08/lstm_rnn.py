...
class LSTMModel(object):
    def __init__(self, is_training, batch_size, num_steps):
        ...
        self.correct_prediction = tf.equal(tf.cast(tf.argmax(logits, 1), tf.int32), tf.reshape(self.targets, [-1]))
...
def run_epoch(session, model, data, train_op, output_log):
    ...
    for step, (x, y) in enumerate(get_enumerate_in_array(data, model.batch_size, model.num_steps)):
        cost, state_fw, state_bw, accuracy, _ = session.run([model.cost, model.final_state_fw, model.final_state_bw, model.accuracy, train_op],
                                                  {
                                                      model.input_data:x,
                                                      model.targets:y,
...
def main():
    ...
    with tf.variable_scope("lottery_train", reuse=None, initializer=initializer):
        train_model = LSTMModel(True, TRAIN_BATCH_SIZE, TRAIN_NUM_STEP)

    with tf.variable_scope("lottery_train", reuse=True, initializer=initializer):
        eval_model = LSTMModel(False, EVAL_BATCH_SIZE, EVAL_NUM_STEP)
    ...
    with tf.Session() as session:
        ...
        for i in range(NUM_EPOCH):
            ...
            valid_perplexity,accuracy = run_epoch(session, eval_model, valid_data, tf.no_op(), False)
            print("Epoch: %d Validation Perplexity: %.3f" % (i + 1, valid_perplexity))
            # if i % 10 == 0:
            #    for i in range(len(train_costs)):
            #        print("loss on training is %.6f" % (train_costs[i]))
            if (i >= 500 and i % 500 == 0) or (valid_perplexity < 10 and i % 20 == 0) or (accuracy > 0.9 and i % 20 == 0):
